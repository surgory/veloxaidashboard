...
import { Copy, Search, Ban, Trash2, X, RefreshCw, Loader2, Key } from "lucide-react";
import { useToast } from "@/hooks/use-toast";
import { Tooltip, TooltipContent, TooltipTrigger } from "@/components/ui/tooltip";
import { supabase } from "@/lib/supabase";
import { sendRevokedAlert } from "@/lib/discord";
...
export default function AllLicenses() {
  ...
  async function fetchLicenses() {
    ...
    if (error) toast({ title: "Error loading licenses", description: error.message, variant: "destructive" });
    setLicenses(data || []);
    setLoading(false);
  }
  ...
  async function revokeLicense(key: string) {
    ...
    const { error } = await supabase.from("licenses").update({
      status: "revoked",
      revoked_by_discord_id: discord?.id || null,
      revoked_by_discord_name: discord?.username || null,
    }).eq("key", key);
    if (error) {
      toast({ title: "Failed to revoke", description: error.message, variant: "destructive" });
    } else {
      toast({ title: "License revoked", description: key });
      setLicenses(prev => prev.map(l => l.key === key ? { ...l, status: "revoked", revoked_by_discord_id: discord?.id || null, revoked_by_discord_name: discord?.username || null } : l));
      sendRevokedAlert({
        key, owner_name: license?.owner_name || "Unknown",
        admin_name: discord?.username || "Unknown",
      });
      logAuditAction(discord?.id || "unknown", discord?.username || "unknown", "revoke", key, `Owner: ${license?.owner_name || "N/A"}`);
    }
  ...
  async function deleteLicense(key: string) {
    ...
    if (error) {
      ...
      logAuditAction(discord?.id || "unknown", discord?.username || "unknown", "delete", key);
    }
    setActionLoading(null);
  }
  ...
  const handleStatusFilter = (status: string) => setStatusFilter(prev => prev === status ? "all" : status);
                ...
                {filtered.map((l) => (
                  ...
                  >
                    ...
                    <TableCell className="hidden md:table-cell text-xs text-muted-foreground">
                      {l.generated_by_discord_name || "—"}
                      {l.status === "revoked" && l.revoked_by_discord_name && (
                        <span className="block text-red-400">Revoked by: {l.revoked_by_discord_name}</span>
                      )}
                    </TableCell>
                    <TableCell>
